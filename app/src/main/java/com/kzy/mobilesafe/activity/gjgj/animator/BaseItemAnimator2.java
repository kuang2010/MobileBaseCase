 ...
 * date: 2020/5/4
 * time: 15:34
 * desc: 完全copy from 低版本的DefaultItemAnimator，private方法 改为 public方法给子类重写
 */
public class BaseItemAnimator2 extends SimpleItemAnimator {
    ...
    public void animateRemoveImpl(final ViewHolder holder) {
        ...
        final ViewPropertyAnimatorCompat animation = ViewCompat.animate(view);//view.animate(); 开启一个属性动画
        mRemoveAnimations.add(holder);//DefaultItemAnimator中执行动画是将所有item的将要执行的动画放入ArrayLists，一起执行他们
        //下面就是动画的逻辑了，这里设置了一个执行时间，执行了一个alpha动画，就是这么简单！
        animation.setDuration(getRemoveDuration())
                .alpha(0).setListener(new VpaListenerAdapter() {
            @Override
            public void onAnimationStart(View view) {
                dispatchRemoveStarting(holder);// 通知动画开始
            }
     ...
     * {@inheritDoc}
     * <p>
     * If the payload list is not empty, BaseItemAnimator2 returns <code>true</code>.
     ...
     * then BaseItemAnimator2 will call {@link #animateMove(ViewHolder, int, int, int, int)} and
